...
# The original script had transformers and tensorflow_hub, which are no longer strictly needed
# for the simplified PUBMEDBERT (using SBERT wrapper) but kept for context if needed later.
from transformers import AutoTokenizer, AutoModel
import tensorflow_hub as hub
import tensorflow as tf

# 🛑 CRITICAL FIX: Prevent TF from grabbing all GPU RAM (Kept from original)
...
# --- HYBRID CONFIG ---
# The Solr fields for hybrid search
VECTOR_FIELD = "vector_pubmedbert"
BM25_FIELD = "text_bm25" # Assumed field for BM25 search
# Alphas to test (0.10 increments from 0.30 to 0.70)
...
# --- OLLAMA (Functions remain the same) ---
...
# --- JSON EXTRACTION HELPER (Function remains the same) ---
...
# --- QUERY GENERATION (Function remains the same) ---
...
# --- EMBEDDING (PUBMEDBERT ONLY) ---
print("⏳ Starting PubMedBERT Embedding Pipeline...")

# We will store only the PUBMEDBERT vectors
QUERY_VECS = {}

def get_pubmedbert_embeddings(texts):
    print("   🟣 Loading PubMedBERT...")
    # Using SentenceTransformer for easy loading of common models like NeuML/pubmedbert-base-embeddings
    model = SentenceTransformer('NeuML/pubmedbert-base-embeddings', device=DEVICE)
    vecs = model.encode(texts, convert_to_numpy=True)
    del model
    return vecs

# Run only the PUBMEDBERT job
print("🚀 Processing pubmedbert...")
try:
    # Key remains 'pubmedbert' for consistency with Solr field name 'vector_pubmedbert'
    QUERY_VECS['pubmedbert'] = get_pubmedbert_embeddings(QUERIES_TEXT)
except Exception as e:
    print(f"❌ Error embedding pubmedbert: {e}")
    QUERY_VECS['pubmedbert'] = []
...
# List of only the one vector type (key)
all_vtypes = list(QUERY_VECS.keys())
print("✅ All queries embedded.")

# --- HYBRID SEARCH FUNCTION ---

def search_solr_single(query_str, vtype, q_vec, is_vector_search=True):
    """Performs a single Solr search (either vector or BM25) and returns a list of (doc_id, score)"""
    search_field = VECTOR_FIELD if is_vector_search else BM25_FIELD
    
    if is_vector_search:
        # Vector search query template
        vec_str = "[" + ",".join(map(str, q_vec)) + "]"
        query_params = {
            "q": f"{{!knn f={search_field} topK={TOP_K}}}{vec_str}",
            "fl": "drug_id,brand_name,purpose,indications_and_usage,warnings,score",
            "rows": TOP_K
        }
    else:
        # BM25 search query template
        query_params = {
            "q": f"{search_field}:{query_str}",
            "fl": "drug_id,brand_name,purpose,indications_and_usage,warnings,score",
            "rows": TOP_K
        }
    ...
    try:
        ...
        print(f"❌ Solr search error for {vtype} ({'Vector' if is_vector_search else 'BM25'}): {e}")
        return []

def search_hybrid(query_str, q_vec, alpha, query_idx):
    """Hybrid search using RRF (Reciprocal Rank Fusion) with a weight 'alpha'"""
    
    # 1. Perform parallel searches
    print(f"  📡 Starting hybrid search (alpha={alpha:.2f})")
    
    vector_docs = search_solr_single(query_str, 'pubmedbert', q_vec, is_vector_search=True)
    bm25_docs = search_solr_single(query_str, 'bm25', q_vec, is_vector_search=False)
    
    # 2. RRF Fusion (using rank instead of score)
    
    # RRF constant k
    k_rrf = 20 
    
    # Document scores dictionary: {drug_id: score}
    fused_scores = {}
    
    # Function to get document scores from a result set
    def apply_rrf(docs, rank_mod):
        for rank, doc in enumerate(docs, 1):
            drug_id = str(doc.get('drug_id'))
            
            # Reciprocal Rank calculation: 1 / (k_rrf + rank)
            rr = 1 / (k_rrf + rank)
            
            # Apply weighted RRF score: score = alpha * RRF_vector + (1-alpha) * RRF_bm25
            # We use rank_mod (alpha or 1-alpha) here
            current_score = fused_scores.get(drug_id, 0)
            fused_scores[drug_id] = current_score + rank_mod * rr
            
            # Store the original doc only once, but update the score to the final RRF score
            doc['fused_score'] = fused_scores[drug_id]
            
    # Apply RRF contributions
    # Vector score contribution: alpha
    apply_rrf(vector_docs, alpha)
    # BM25 score contribution: 1 - alpha
    apply_rrf(bm25_docs, (1 - alpha))
    
    # 3. Consolidate and Rank
    
    # Get all unique documents from both result sets
    all_docs = {}
    for doc in vector_docs + bm25_docs:
        doc_id = str(doc.get('drug_id'))
        if doc_id:
            if doc_id not in all_docs:
                # Add original metadata and the new fused_score
                doc['score'] = fused_scores[doc_id] # Overwrite Solr score with the RRF score
                all_docs[doc_id] = doc
            else:
                # If the doc already exists, update its score if it was added from the other search later
                all_docs[doc_id]['score'] = fused_scores[doc_id]
                
    # Sort by the final RRF score (descending)
    ranked_docs = sorted(list(all_docs.values()), key=lambda x: x['score'], reverse=True)
...
# --- PER-METHOD JUDGMENT SYSTEM (Functions judge_documents_per_method and extract_json_from_text remain the same) ---
print("⚖️ Setting up PER-METHOD Judgment System...")

RESULTS = []
METHOD_QRELS = {}  # {query_idx: {"method_name": {"drug_id1": 1, "drug_id2": 1, ...}}}
...
def collect_and_judge_query_per_method(i):
    """Collect and judge documents for EACH ALPHA separately (The 'method' is now the alpha)"""
    query = QUERIES_TEXT[i]
    # We use the PUBMEDBERT vector since it's the only one
    q_vec = QUERY_VECS['pubmedbert'][i]
    print(f"\n🔍 PER-METHOD JUDGE Q{i}: '{query}'")
    
    query_qrels = {}
    
    # --- Iterate over the ALPHAS for hybrid search ---
    for alpha in ALPHAS:
        method_name = f"pubmedbert-bm25-a{alpha:.2f}"
        
        # Perform the hybrid search
        hybrid_docs = search_hybrid(query, q_vec, alpha, i)
        
        if hybrid_docs:
            # Store results
            RESULTS.append({"algo": "Hybrid-RRF", "vtype": method_name, "q_idx": i, "docs": hybrid_docs})
            print(f"  📥 {method_name}: Found {len(hybrid_docs)} documents")
            
            # Judge documents for THIS METHOD ONLY (in chunks of 5)
            method_qrels = judge_documents_per_method(query, hybrid_docs, f"{method_name}-Q{i}")
            query_qrels[method_name] = method_qrels
        else:
            print(f"  ⚠️ {method_name}: No documents found")
            query_qrels[method_name] = {}
    
    return i, query_qrels

print("\n" + "="*80)
print("STARTING HYBRID PER-METHOD JUDGING PROCESS")
print("="*80)

# The set of methods to evaluate now includes all the alpha values
all_methods = [f"pubmedbert-bm25-a{a:.2f}" for a in ALPHAS]
...
for method in all_methods:
    total_relevant = sum(len(METHOD_QRELS[i].get(method, {})) for i in range(len(QUERIES_TEXT)))
    print(f"{method}: {total_relevant} relevant documents")
...
per_query_metrics = {method: {} for method in all_methods}
...
for method_name in all_methods:
    ...
    for i, q in enumerate(QUERIES_TEXT):
        ...
        for r in RESULTS:
            if r['q_idx'] == i and r['vtype'] == method_name:
                docs = r['docs']
                ...
                print(f"  Found {len(docs)} documents for {method_name}")
                ...
                per_query_metrics[method_name][i] = {**{f"p_at_{k}": p_at_k_scores[k] for k in P_AT_K_LIST}, "ap": ap}
...
# --- 11-POINT INTERPOLATED PRECISION-RECALL CURVES with UNIFIED judgments ---
print("\n" + "="*80)
print("11-POINT INTERPOLATED PRECISION-RECALL CURVES (HYBRID ALPHAS)")
print("="*80)

# Calculate 11-point interpolated precision for each algorithm and query (Function remains the same)
...
# Colors and styles for each alpha value
colors = {method: cm.viridis(i/len(all_methods)) for i, method in enumerate(all_methods)}
line_styles = {method: '-' for method in all_methods}


# For each method (alpha), plot ONLY the AVERAGE curve
for method_name in all_methods:
    algo_name = method_name
    ...
    if all_scores and sum(all_labels) > 0:
            ...
            # Get average MAP
            map_scores_for_algo = [m["ap"] for m in metrics_data if m["vtype"] == method_name]
            avg_map = np.mean(map_scores_for_algo) if map_scores_for_algo else 0
            
            # --- NEW: Calculate Average for ALL P@K values (5, 10, 20) ---
            p_at_k_strings = []
            for k in P_AT_K_LIST:
                scores = [m[f"p_at_{k}"] for m in metrics_data if m["vtype"] == method_name]
            ...
            plt.plot(recall_levels, interp_precisions, 
                     ...
                     # Label now reflects the alpha value clearly
                     label=f"Hybrid-a{float(method_name.split('a')[1]):.2f}: MAP={avg_map:.3f}, {p_legend_str}",
                     linewidth=3,
...
plt.title(f"Hybrid PubMedBERT+BM25 P-R Curve (Alpha Comparison)\n({len(QUERIES_TEXT)} queries, P@{P_AT_K_LIST})", fontsize=14)
...
# Save the PR curve
plt.savefig("precision_recall_curve_hybrid_alpha_compare.png", dpi=150)
print("\n✅ 11-point interpolated Precision-Recall curve saved as 'precision_recall_curve_hybrid_alpha_compare.png'.")
plt.show()

# --- FINAL RESULTS TABLE (MAP COMPARISON) with PER-METHOD judgments ---
print("\n" + "="*80)
print("🏆 FINAL HYBRID RESULTS TABLE (Alpha Comparison)")
print("="*80)
...
for method_name in all_methods:
    algo_metrics = [m for m in metrics_data if m["vtype"] == method_name]
    if algo_metrics:
        table_row = {"Algorithm": method_name.upper()}
        for k in P_AT_K_LIST:
...
df_table['Alpha_Sort'] = df_table['Algorithm'].apply(lambda x: float(x.split('-A')[1]))
df_table = df_table.sort_values(by='Alpha_Sort').drop(columns=['Alpha_Sort'])
print(df_table.to_string(index=False))

# --- PER-QUERY RESULTS FOR DEBUGGING ---
print("\n" + "="*80)
print("📊 PER-QUERY RESULTS (HYBRID ALPHA)")
print("="*80)
...
# Save final results
with open("final_results_hybrid_alpha_compare.json", "w") as f:
    json.dump({
        "config": {
            "top_k": TOP_K,
            ...
            "judgment_method": "PER-METHOD-CHUNKED-HYBRID",
...
print("\n✅ Final results saved as 'final_results_hybrid_alpha_compare.json'.")
print("\n" + "="*80)
print("HYBRID ALPHA EVALUATION COMPLETE! (PUBMEDBERT + BM25)")
print("="*80)